...
static void cleanup(void) {
    ...
    for (; i <= LISTMAX; i++)
    {
        ...
        {
            block_t *temp = (void *)(m_root->body.next);
            coalesce(m_root);
            m_root = temp;
            count++;
        }
...
static double ratio(void) {
    size_t heap = mem_heap_hi() - mem_heap_lo();
    return allocSize / heap;
}
...
int mm_init(void) { 
    ...
    allocSize += OVERHEAD + MIN_BLOCK_SIZE * (LISTMAX + 1);
    return 0;
}
/* $end mminit */
