...
static void seg_list_freeness_consistency(block_t *bp);
/* Checks if all the blocks in a particular segmented list are free or not*/
static void seg_list_size_consistency(block_t *bp, int blockNum);
...
int calcList(size_t blockSize) {
    ...
    //     return 0;
    // }
    // else if (blockSize == UINT32_MAX) {
    //     return 27;
    // }
    // else {
    //     return loga(blockSize) - 4;
    // }
}
...
static void print_seg_list()
{
    ...
    for (blockNum = 0; blockNum <= LISTMAX; blockNum++)
    {
        block_t *m_root = (void *)segList + MIN_BLOCK_SIZE * blockNum;
        printf("The block Number %d which is located at", blockNum);
        printf("%p has the address : [%p] \n", m_root, m_root->body.next);
        seg_list_freeness_consistency(m_root);
        seg_list_size_consistency(m_root, blockNum);
    }
}
